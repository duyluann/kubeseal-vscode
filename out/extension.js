...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('kubeseal.encrypt', (uri) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kubeseal.decrypt', (uri) => {
        vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kubeseal.setCertPath', () => {
        vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kubeseal.encodeBase64', (uri) => {
        vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kubeseal.decodeBase64', (uri) => {
        vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
...
async function encryptSecret(uri, progress) {
    try {
        ...
        const kubesealPath = config.get('kubesealPath', 'kubeseal');
        // Check if certificate path is set
        if (!certPath) {
            progress?.report({ message: "Certificate path not set. Asking user..." });
            const result = await vscode.window.showInformationMessage('Certificate path not set. Would you like to set it now?', 'Yes', 'No');
            if (result === 'Yes') {
                await setCertificatePath(progress);
                // Fetch updated config
                const newConfig = vscode.workspace.getConfiguration('kubeseal');
                certPath = newConfig.get('certPath', '');
            }
            if (!certPath) {
        ...
        const openResult = await vscode.window.showInformationMessage('Would you like to open the encrypted file?', 'Yes', 'No');
        if (openResult === 'Yes') {
...
async function decryptSecret(uri, progress) {
    try {
        progress?.report({ message: "Reading secret file..." });
        const filePath = uri.fsPath;
        const config = vscode.workspace.getConfiguration('kubeseal');
        ...
        const openResult = await vscode.window.showInformationMessage('Would you like to open the decrypted file?', 'Yes', 'No');
        if (openResult === 'Yes') {
...
async function setCertificatePath(progress) {
...
async function encodeBase64Values(uri, progress) {
    try {
...
async function decodeBase64Values(uri, progress) {
    try {
        progress?.report({ message: "Reading file..." });
        const filePath = uri.fsPath;
        const inputContent = fs.readFileSync(filePath, 'utf8');
